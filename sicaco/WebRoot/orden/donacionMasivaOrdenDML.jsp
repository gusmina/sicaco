      ...
      <td colspan="4" align="center">
      	<logic:present name="boton">
        	...
        	<logic:equal name="boton" value="0">
	      		...
	      		<input type="button" value="<bean:message key="lbl.ordcom.massiveDon"/>"
						onclick="handlerMassiveDonButton();">
				<logic:equal name="admin" value="ADMINISTRADOR">
					&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="button" value="<bean:message key="lbl.ordcom.cleanDon"/>"
						onclick="handlerCleanDonButton();">
				</logic:equal>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="button" value="Limpiar"
					onclick="cleanFields('formId');">
				<logic:present name="imprimirMass">
					<html:submit property="accion" onclick="$('#pageId').val('0');">
						<bean:message key="cmd.ordcom.imprimirOrdenes"/>
					</html:submit>
				</logic:present>
	        </logic:equal>
        </logic:present>
...
<script type="text/javascript">
	...
	function handlerCleanDonButton(){
	var msgText = '<h3 style="color: red;">!!Limpieza de Base de Datos y Creacion de Ordenes!!</h3>'+
   				 '<p style="font-size: 1.4em; font-weight: bold;">Esta completamente seguro que desea eliminar donaciones anteriores y reiniciar el correlativo para esta donacion masiva ?';	
				$.prompt(msgText,{
      				buttons:{Ok:true,Cancel:false}, 
      				prefix:'brownJqi',
      				callback : function(v,m){
      					if(v == true){
      						$('#accionId').val('cleanDon');
      						$('#pageId').val('0');
							$('#formId')[0].submit();
      					}
      				}
	...
	function handlerMassiveDonButton(){
	var msgText = '<h3 style="color: red;">!!Creacion Masiva de Ordenes!!</h3>'+
   				 '<p style="font-size: 1.4em; font-weight: bold;">Esta completamente seguro que desea crear ordenes para todos los Asociados ?';	
				$.prompt(msgText,{
      				buttons:{Ok:true,Cancel:false}, 
      				prefix:'brownJqi',
      				callback : function(v,m){
      					if(v == true){
      						$('#accionId').val('massiveDon');
      						$('#pageId').val('0');
							$('#formId')[0].submit();
      					}
      				}
